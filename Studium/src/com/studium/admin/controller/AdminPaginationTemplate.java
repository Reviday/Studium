...
public class AdminPaginationTemplate {
	 ...
	 */
	public AdminPaginationTemplate (HttpServletRequest request, int totalData, String URLmapping, String method)  
	{
		// 페이지 설정
		try {
	...
	private String setPageBar(String URLmapping, Map<String, String> attributes,String method) 
	{
		// pageBar 소스코드 작성!
		...
		if(pageNo <= pageBarSize) {
			...
			pageBar += "<li><a href='javascirpt:void(0)' onclick='"+ method + "();' class='first'>처음 페이지</a></li>";
		}
		...
		if(pageNo == 1) {
			...
			pageBar += "<li><a href='javascirpt:void(0)' onclick='" + method + "("+(pageNo-1)+");' class='arrow left'><<</a></li>";
		}
		...
		while (!(pageNo > pageEnd || pageNo > totalPage)) {
			if (pageNo == cPage) {
				...
				pageBar += "<li><a href='javascirpt:void(0)' onclick='"+method+"("+pageNo+");' class='num'>"+pageNo+"</a></li>";
			}
			pageNo++;
		}
		...
		if (pageNo > totalPage) {
			...
			pageBar += "<li><a href='javascirpt:void(0)' onclick='" + method + "("+pageNo+");' class='arrow right'>>></a></li>";
		}
		...
		if((pageEnd-1)/pageBarSize == (totalPage-1)/pageBarSize) {
			...
			pageBar += "<li><a href='javascirpt:void(0)' onclick='" + method + "("+pageNo+");' class='last'>끝 페이지</a></li>";
		}
	...
	public String getPageBar() {
		// 페이지 바 만들기
		pageBar=setPageBar(URLmapping, attributes, method);
		return pageBar;
	}
