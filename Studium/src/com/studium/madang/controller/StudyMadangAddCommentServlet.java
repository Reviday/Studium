...
/**
 * Servlet implementation class StudyMadangAddCommentServlet
 */
@WebServlet("/madang/studyAddComment")
public class StudyMadangAddCommentServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public StudyMadangAddCommentServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2차 로그인 체크. 
		if(!new LoginCheck().doLoginCheck(request, response, 1002)) return;
		...
		int result=new StudyMadangCmtService().insertComment(cmt);
		
		String view="";
		if(result>0) {
			view="/madang/studyMadangView?madangNo="+madangNo+"&cPage="+cPage;
			request.setAttribute("choice", "자랑마당");
			request.setAttribute("choiceSub", request.getParameter("choiceSub"));
		} else {
			String msg="댓글 작성에 실패하였습니다.";
			String loc="/madang/studyMadangView?madangNo="+madangNo+"&cPage="+cPage;
