<%@page import="com.studium.madang.model.vo.QuestionMadangCmt"%>
...
<%
	...
	Map<String, QuestionMadang> preNext=(Map<String, QuestionMadang>)request.getAttribute("preNext");
...
<section id="content-area" class="row">
	<div class="col-xs-4 col-bm-3 col-md-3 col-lg-2"
	...
	<!-- 게시글 View Main -->
	<div id="main-area" class="col-xs-6 col-bm-6 col-md-7 col-lg-7">
		...
		<div class="madang-view">
			...
			<div class="inbox">
				...
				<div class="tit-box">
					...
					<div class="fl">
						<table role="presentation" cellspacing="0" cellpadding="0"
							...
							<tbody>
								<tr valign="top">
									<td><span class="b m-tcol-c"><%=qm.getMadangTitle()%></span></td>
					...
					<div class="fr">
						<table role="presentation" cellspacing="0" cellpadding="0"
							...
							<tbody>
								<tr>
									<td></td>
									<td class="m-tcol-c date"><%=format.format(qm.getMadangRegisterDatetime())%></td>
								</tr>
							</tbody>
						</table>
					</div>
											...
											<tbody>
												<tr>
													<td class="profile_img"><a href="#"> <!-- 프로필이미지가 없을 경우 다음 이미지로 대채한다. -->
															<img src="<%=request.getContextPath()%>/upload/myPage/<%=qm.getProfilePath()%>" width="24" height="24"
													...
													<td class="p-nick">
														...
														<a href="#" class="m-tcol-c b"><%=qm.getMadangWriterName()%>
															<!-- 아이디 별표(*)처리 --> (<%=qm.getMadangWriterEmail().substring(0, 4)%>****)</a>
													</td>
												</tr>
											</tbody>
										</table>
				...
				<div class="bottom-box">
					...
					<div class="fr">
						<%
							if(loginMember!=null && (loginMember.getMemNo()==qm.getMadangWriterUid() || loginMember.getMemCode()=='M')) {
								%>
									<div class="post_btns font-family-type1" style="display: inline;">
										<a href="#" id="writeFormBtn"
											onclick="fn_madangUpdate('<%=loginMember!=null?loginMember.getMemNo():null%>','<%=request.getContextPath()%>','question','<%=qm.getMadangNo() %>','<%=choice%>','<%=choiceSub%>','<%=cPage%>'); return false;"
											class="btn_type1 post_write _rosRestrict" style="color:#000">수정하기</a>
									</div>
									<div class="post_btns font-family-type1" style="display: inline;">
										<a href="#" id="writeFormBtn"
											onclick="fn_madangDelete('<%=loginMember!=null?loginMember.getMemNo():null%>','<%=request.getContextPath()%>','question','<%=qm.getMadangNo() %>','<%=choice%>','<%=choiceSub%>','<%=cPage%>'); return false;"
								...
								<tr>
									...
									<td class="i3">
										...
										<div class="u_cbox_btn_upload _submitBtn">
											...
											onclick="fn_addComment('<%=loginMember!=null?loginMember.getMemNo():null%>', 'question', '<%=request.getContextPath() %>','<%=REMOTE_ADDR%>','<%=qm.getMadangNo()%>','<%=loginMember!=null?loginMember.getMemNo():""%>','<%=loginMember!=null?loginMember.getMemUserEmail():""%>','<%=loginMember!=null?loginMember.getMemName():""%>','<%=cPage%>', '<%=choice%>', '<%=choiceSub%>'); return false;">
											등록</a>													
										</div>
									</td>
								</tr>
								...
								<div id="comment">
									...
									<%
										if(!list.isEmpty()) {
											...
											for(QuestionMadangCmt cmt : list) {
						                                ...
						                                <div class="rp_general cng-container">
						                                    <div class="cng-header">
                                                                ...
                                                                %>
			 			                                        ...
			 			                                        <span class="timeago dt-published ie-dotum" title="">
			 			                                        	<a href="#" class="m-tcol-c ablock" id="edit_cmt_<%=cmt.getCmtNo()%>"
			 			                                        		onclick="fn_deleteComment('<%=cmt.getCmtNo()%>','<%=loginMember!=null?loginMember.getMemNo():null%>', 'question', '<%=request.getContextPath() %>','<%=qm.getMadangNo()%>','<%=loginMember!=null?loginMember.getMemNo():""%>','<%=cPage%>', '<%=choice%>', '<%=choiceSub%>'); return false;">
			 			                                        		삭제</a>
			 			                                        </span>
			 			                                        <span class="m-tcol-c" style="opacity: 0.3">|</span>
			 			                                        <%
																		...
																		<tbody>
																			<tr>
																				...
																				<td class="i3">
																					...
																					<div class="u_cbox_btn_upload _submitBtn">
																						<a href="#" class="u_cbox_txt_upload _submitCmt"
																						onclick="fn_updateComment('<%=cmt.getCmtNo()%>','<%=loginMember!=null?loginMember.getMemNo():null%>', 'question', '<%=request.getContextPath() %>','<%=REMOTE_ADDR%>','<%=qm.getMadangNo()%>','<%=loginMember!=null?loginMember.getMemNo():""%>','<%=loginMember!=null?loginMember.getMemUserEmail():""%>','<%=loginMember!=null?loginMember.getMemName():""%>','<%=cPage%>', '<%=choice%>', '<%=choiceSub%>'); return false;">
																						수정</a>													
																					</div>
																				</td>
																			</tr>
                                                            ...
                                                            <div class="rp_admin cng-container">
                                                                <div class="cng-header">
                                                                    ...
                                                                    %>
				 			                                        ...
				 			                                        <span class="timeago dt-published ie-dotum" title="">
				 			                                        	<a href="#" class="m-tcol-c ablock" id="edit_cmt_<%=cmt.getCmtNo()%>"
				 			                                        		onclick="fn_deleteReply('<%=cmt.getCmtNo()%>','<%=cmt.getCmtParent() %>','<%=cmt.getCmtSort()%>','<%=loginMember!=null?loginMember.getMemNo():null%>', 'question', '<%=request.getContextPath() %>','<%=qm.getMadangNo()%>','<%=loginMember!=null?loginMember.getMemNo():""%>','<%=cPage%>', '<%=choice%>', '<%=choiceSub%>'); return false;">
				 			                                        		삭제</a>
				 			                                        </span>
				 			                                        <%
                                                                    	} else {
																		...
																		<tbody>
																			<tr>
																				...
																				<td class="i3">
																					...
																					<div class="u_cbox_btn_upload _submitBtn">
																						<a href="#" class="u_cbox_txt_upload _submitCmt"
																						onclick="fn_updateReply('<%=cmt.getCmtNo()%>','<%=loginMember!=null?loginMember.getMemNo():null%>', 'question', '<%=request.getContextPath() %>','<%=REMOTE_ADDR%>','<%=qm.getMadangNo()%>','<%=loginMember!=null?loginMember.getMemNo():""%>','<%=loginMember!=null?loginMember.getMemUserEmail():""%>','<%=loginMember!=null?loginMember.getMemName():""%>','<%=cPage%>', '<%=choice%>', '<%=choiceSub%>'); return false;">
																						수정</a>													
																					</div>
																				</td>
																			</tr>
		...
		<div class="remote-area">
			...
			<div class="remote-area-body">
				<div class="btn btn_upper">
					...
					<p>
						<a href="#" class="m-tcol-c <%=preNext.get("next").getMadangNo()!=0?"":"disabled"%>" 
							onclick="location.href='<%=request.getContextPath()%>/madang/questionMadangView?madangNo=<%=preNext.get("next").getMadangNo()%>&cPage=<%=cPage%>&choiceSub=<%=choiceSub%>'"> 
				...
				<div class="btn btn_lower">
					<span></span>
					<p>
						<a href="#" class="m-tcol-c" onclick="location.href='<%=request.getContextPath()%>/madang/questionMadangList?cPage=<%=cPage%>&choice=자랑마당&choiceSub=<%=choiceSub%>'">
					...
					<p>
						<a href="#" class="m-tcol-c"  
							<%
								if(loginMember!=null) {
									%>
										onclick="location.href='<%=request.getContextPath()%>/madang/madangWrite?locate=question&cPage=<%=cPage%>&choice=자랑마당&choiceSub=<%=choiceSub%>'"
									<%
								} else {
							%>
				...
				<div class="btn">
					...
					<p>
						<a href="#" class="m-tcol-c <%=preNext.get("prev").getMadangNo()!=0?"":"disabled"%>" 
							onclick="location.href='<%=request.getContextPath()%>/madang/questionMadangView?madangNo=<%=preNext.get("prev").getMadangNo()%>&cPage=<%=cPage%>&choiceSub=<%=choiceSub%>'"> 
