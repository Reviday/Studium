					...
					<blockquote class="testimonial">
						<div class="d-flex author">
							<div id="myProfile-preview">
								...
								%>
								<img
									src="<%=request.getContextPath()%>/upload/myPage/<%=m.getMemPhoto()%>"
									alt="" id="myProfile_y" class="mr-4">
								<%
									} else {
								%>
								<img src="<%=request.getContextPath()%>/img/default_image.png"
									alt="" id="myProfile_n" class="mr-4">
								<%
									}
								%>
							</div>
							<div class="author-info">
								<div>
									...
									<form
										action="<%=request.getContextPath()%>/myPage/changeMyPhoto"
										id="frm_profile_img" method="post"
										enctype="multipart/form-data">

										<input type="hidden" value="<%=m.getMemNo()%>" name="no">
										<input type="file" id="file" class="fileinput" name="myPhoto"
											accept="image/png,image/jpg" /> <label for="file"
											class="btn-2" onClick="">사진변경</label> <input type="button"
											id="changePhoto" class="changePhoto" value="사진변경완료" />
									</form>
									<div style="margin-top: 50px">스타디움은 사람들과 얼굴을 마주하고 오프라인에서
										만남을 가지는 모임입니다. 사진을 통해 회원님에 대해 알려주세요. :)</div>
								</div>
							</div>
					</blockquote>
				</div>
				<div class="myInfo">
					...
					<div class="row">
						<div class="col-5">
							...
							<!--하나라도 null이면 안보이게 처리-->
							<%
								if (m.getMemBirth() != null && m.getMemPhone() != null && m.getMemZipCode() != null
										&& m.getMemCategory1() != null) {
							%>
						...
						<div class="col-7">
							<form action="<%=request.getContextPath()%>/myPage/modifyMember"
								...
								<div class="inputForm">
									...
									<input type="hidden" name="loginMember"
										value="<%=m.getMemUserEmail()%>">
								</div>
								<div class="inputForm">
								...
								<div class="inputForm">
									<input type="password" name="password" id="modipwd"
										class="myI-1" required>
								</div>
								<div class="inputForm">
									<input type="password" name="passwordck" id="modipwdck"
										class="myI-1" required>
								</div>
								...
								%>
								<div class="inputForm">
									<input type="phone" name="phone" id="phone" class="myI-1"
										autocomplete=off value="<%=m.getMemPhone()%>" required>
								</div>
								<div class="inputForm">
									<input type="text" name="birth" id="birth" class="myI-1-f"
										autocomplete=off value="<%=m.getMemBirth()%>" required>
								</div>


								<div class="inputAddress">
									<input type="text" id="sample6_postcode" name="zipcode"
										class="myI-1-add" value="<%=m.getMemZipCode()%>"> <input
										type="button" onclick="sample6_execDaumPostcode()"
										class="myI-1-add btn-address" value="우편번호 찾기"> <input
										type="text" id="sample6_address" name="address1" class="myI-1"
										value="<%=m.getMemAddress1()%>"> <input type="text"
										id="sample6_detailAddress" name="address2" class="myI-1"
										value="<%=m.getMemAddress2()%>">
								</div>
								<div class="inputInteresting">
									...
									<label class="check-label"> <input type="checkbox"
										class="option-input checkbox" name="inter"
										<%if (listM.get(j).getTitleM().equals(m.getMemCategory1())
										|| listM.get(j).getTitleM().equals(m.getMemCategory2())
										|| listM.get(j).getTitleM().equals(m.getMemCategory3())) {%>
										checked <%}%> id="<%=listM.get(j).getCategoryMId()%>"
										value="<%=listM.get(j).getTitleM()%>"> <%=listM.get(j).getTitleM()%>
									</label>
									<%
										}
									...
									<div class="myI-1-info">
										<div>
											<a href="<%=request.getContextPath()%>/myPage/addMyInfo"><img
												src="<%=request.getContextPath()%>/img/peep.png"></a>
										</div>
										<a href="<%=request.getContextPath()%>/myPage/addMyInfo">입력된
											추가정보가 없습니다ㅠㅅ ㅠ <br>추가정보를 입력해주세요!
										</a>
									</div>
									<%
										}
									...
									<input type="submit" 
										...
										type="hidden" name="zipcode" value=null> <input
										type="hidden" name="address1" value=null> <input
										type="hidden" name="address2" value=null> <input
										type="hidden" name="inter" value=null>
