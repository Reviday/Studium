<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%
     List<Fstudy> pList=(List)request.getAttribute("fList");
...
<body>
  ...
  <div class="wrap" style="margin-left:100px;">
    ...
    <div style="width: 900px; padding-top: 50px;">
      ...
      <form method=post action="<%=request.getContextPath()%>/fstudy/search">
        <select class=input1 name="p_area" style="width: 120px; height: 30px;">
          <option value=''>지역 선택</option>
          <option value='강남'>강남</option>
          <option value='건대'>건대</option>d
			...
			<div class="a_cnt1">
				<div data-aos="fade-up" class="col-3 pstudy-card">
					...
					<% } %>
                    	...
                    	if(loginMember!=null) {
                    %>
					<div class="card-bottom"	OnClick="location.href ='<%=request.getContextPath()%>/pstudy/pstudyProduct?pNo=<%=p.getfNo()%>&mNo=<%=loginMember.getMemNo() %>'"
						style="cursor:pointer;">
					<%}else {%>
					<div class="card-bottom"	OnClick="location.href ='<%=request.getContextPath()%>/pstudy/pstudyProduct?pNo=<%=p.getfNo()%>&mNo=0'"
						style="cursor:pointer;">
					<% } %>
